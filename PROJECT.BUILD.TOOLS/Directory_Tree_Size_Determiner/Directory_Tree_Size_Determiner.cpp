...
Directory_Tree_Size_Determiner::Directory_Tree_Size_Determiner(const Directory_Tree_Size_Determiner & orig){
...
void Directory_Tree_Size_Determiner::Determine_Directory_Tree_Size(char * dir_path){
     ...
     for(int i=0;i<sub_dir_num;i++){
         ...
         char * sub_dir = Enumerator.Get_Directory_List_Element(i);

         Directory_Enumerator Loop_Enumerator;

         Loop_Enumerator.Find_Sub_Directories(sub_dir);
         ...
         if(loop_sub_dir > 0){
           ...
           this->Determine_Directory_Tree_Size(sub_dir);
         }
     }
}
